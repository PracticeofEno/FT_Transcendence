...
import { TMP, imageFileFilter, UserStatusType } from "../util";
import { TwoFactorAuthenticationService } from "../auth/2FA/twoFactorAuthentication.service";
import JwtTwoFactorGuard from "../auth/jwt/jwt-two-factor.guard";
...
export class UsersController {
  ...
  async uploadAvatar(@Request() req, @UploadedFile() file: Express.Multer.File) {
    ...
    if (!filename.includes("profile")) {
      fs.unlinkSync(join(process.cwd(), "./users/avatar/" + filename + ".png"));
    }
